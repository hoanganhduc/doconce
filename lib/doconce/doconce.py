...
def typeset_section_numbering(filestr, format):
    ...
    for i in range(len(lines)):
        if lines[i].startswith('========= '):
        	pattern = '========= +\{?(Abstract|Acknowledgements|Acknowledgments|References|Bibliography|Publications)'
        	if re.search(pattern, lines[i]):
        		continue
    		chapter += 1
    		section = subsection = subsubsection = 0
    		lines[i] = re.sub(r'^========= ', '========= %s ' % counter(chapter), lines[i])
        elif lines[i].startswith('======= '):
        	pattern = '======= +\{?(Journal|International Conference|Others \(not included in this thesis\)|References|Bibliography)'
        	if re.search(pattern, lines[i]):
        		continue
    		section += 1
    		subsection = subsubsection = 0
    		lines[i] = re.sub(r'^======= ', '======= %s ' % counter(chapter, section), lines[i])
        elif lines[i].startswith('===== '):
            # Skip exercises
            if option('examples_as_exercises'):
                pattern = '===== +\{?(Exercise|Project|Problem|Example|References|Bibliography)'
            else:
                pattern = '===== +\{?(Exercise|Project|Problem|References|Bibliography)'
            if re.search(pattern, lines[i]):
                continue
            subsection += 1
            subsubsection = 0
            lines[i] = re.sub(r'^===== ', '===== %s ' % counter(chapter, section, subsection), lines[i])
#            lines[i] += '\n<!-- subsection:%s -->' % counter(chapter, section, subsection)
            ...
            lines[i] = re.sub(r'^=== ', '=== %s ' %
                              ...
                              lines[i])
#            lines[i] += '\n<!-- subsection:%s -->' % counter(chapter, section, subsection, subsubsection)
    return '\n'.join(lines)
