...
""" % (label, top_hr, image, caption, bottom_hr)
       ...
       # with two <linebreak>:
       s = '<br /><center><p>%s</p></center><br />' % image
       return s
...
def html_ref_and_label(section_label2title, format, filestr):
    ...
    for label in section_label2title:
        title = section_label2title[label]
        if re.match(r'\d+', title[:title.find(' ')]) or re.match(r'\d+.\d+', title[:title.find(' ')]) or re.match(r'\d+.\d+.\d+', title[:title.find(' ')]):
        	pos = title.find(' ')
        	filestr = filestr.replace('ref{%s}' % label, '<a href="#%s">%s</a>' % (label, title[:pos]))
        else:
		    filestr = filestr.replace('ref{%s}' % label, '<a href="#%s">%s</a>' % (label, title))
    ...
    # Should have <div id=""> type of labels too
...
#    			fig_no += 1
				...
				to_ = caption_start + 'Figure %s: ' % fig_no + caption
