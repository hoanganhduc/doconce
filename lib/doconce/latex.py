        ...
        if filename_ext == '.pgf' or filename_ext == '.tikz':
            ...
            if latex_style == 'tufte-book':
                if frac <= tufte_frac_limit4marginfig:
                    ...
                    includeline = r'\centerline{\includegraphics[scale=%s]{%s}}' % (frac, filename)
                ...
                if sidecaption == 1:
                    includeline = r'\includegraphics[scale=%s]{%s}' % (frac, filename)
                else:
                    includeline = r'\centerline{\includegraphics[scale=%s]{%s}}' % (frac, filename)
...
def latex_title(m):
    ...
    if latex_style == "JAIST_thesis":
        text = ''
    elif latex_style in ("Springer_sv", "Springer_T2", "Springer_T4",
...
""" % vars()
    elif title_layout == "Springer_llncs":
        # No blue section here since style here is governed by Springer
        text += r"""
\mainmatter
\title{%(title)s}
...
\titlerunning{%(short_title)s}
""" % vars()
    elif title_layout == "JAIST_thesis":
		text += r"""
\title{%(title)s}
""" % vars()
...
\author{"""
        ...
        for a, i, e in authors_and_institutions:
            ...
            a_text += inst
            author_command.append(a_text)
            emails_local.append(email_split(e).local)
            emails_domain.append(email_split(e).domain)
        emails = []
        for dom in emails_domain:
			ind = [i for i, x in enumerate(emails_domain) if x == dom]
			if len(ind) == 1:
				s = emails_local[ind[0]] + '@' + emails_domain[ind[0]]
				emails.append(s)
			else:
				s = '\{'
				for i in ind:
        ...
        text += r'\institute{' + '\n\\and\n'.join(
            [inst + '\n\\email{%s}' % emails[i] for i, inst in enumerate(institutions)]) + '}'
...
def latex_abstract(m):
    ...
    abstract = ''
    if title_layout in ('Springer_collection', 'Springer_llncs'):
        abstract += r"""
\abstract{
%(text)s
}
...
def define(FILENAME_EXTENSION,
    ...
    if latex_style not in ('std',
                           ...
                           'siamltex', 'siamltexmm',
                           'elsevier',
                           'Koma_Script', 'JAIST_thesis', 'Springer_llncs'):
...
"""
    if latex_style in ('Springer_sv', 'Springer_lnup', 'tufte-book',
                       'Springer_T2', 'Springer_T4', 'siambook', 'Springer_llncs'):
        toc_part += r"""
\mainmatter
"""
...
"""
    if latex_style in ('elsevier', 'JAIST_thesis'):
        INTRO['latex'] += r"""
\usepackage{amsmath,
			amsthm, % The amsthm package provides extended theorem environments
			amssymb % The amssymb package provides various useful mathematical symbols
		}
"""
    elif latex_style == 'tufte-book':
        INTRO['latex'] += r"""
...
"""
    elif latex_style not in ('Springer_lnup', 'Springer_sv', 'elsevier', 'JAIST_thesis'):
        INTRO['latex'] += r"""
\usepackage{relsize,epsfig,makeidx,color,setspace,amsmath,amsfonts,amssymb}
...
"""
    ...
    if latex_style not in ('Springer_lnup', 'Springer_sv', 'elsevier', 'JAIST_thesis') and latex_font != 'utopia':
        INTRO['latex'] += r"""
...
""" % vars()
    ...
    if chapters and latex_style not in ("JAIST_thesis"):
        toc_depth += 1
    INTRO['latex'] += r"""
...
""" % vars()
    ...
    if 'FIGURE:' in filestr:
        if latex_style not in ('Springer_sv', 'Springer_lnup', 'elsevier', 'JAIST_thesis'):
            INTRO['latex'] += r"""
...
"""
    ...
    if latex_style not in ('elsevier', 'JAIST_thesis'):
        INTRO['latex'] += r"""
...
% USER PREAMBLE
"""    
    if title_layout != 'beamer' and latex_style not in ('elsevier', 'JAIST_thesis'):
